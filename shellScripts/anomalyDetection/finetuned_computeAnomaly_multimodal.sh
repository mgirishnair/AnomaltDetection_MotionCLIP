...
#SBATCH --job-name="computeAnomaly_split_classContrasSplit"
...
#SBATCH --array=0-5
#SBATCH --output=/scratch/mgirishnair/SLURM_logs/compute_anomaly/%x_%A_%a.out
...
python /scratch/mgirishnair/MotionCLIP_experiment/motionCLIP_anomalyDetection_computeAnomaly_multiModal.py \
  --embeddings_path "${EMBEDDINGS_PATH}" \
  --split "${SPLIT_NAME}" \
  --threshold_percentile 95 \
  --output_path "/scratch/mgirishnair/MotionCLIP_experiment/results/multimodal/classwiseGaussian/contrastiveSplit/final/anomaly_results_${SPLIT_NAME}.txt"
