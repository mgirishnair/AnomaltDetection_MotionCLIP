...
def direction_anchor_contrastive_loss(
    ...
    loss_t2m = -(positive.t() * log_t2m).sum(dim=1) / positive.t().sum(dim=1).clamp_min(1.0)
    return 0.5 * (loss_m2t.mean() + loss_t2m.mean())
class ResidualGeometryMLP(nn.Module):
    """Tiny residual-geometry classifier for fully-unseen generalization.
    It does not see the full 512-d residual. It only sees four geometry scalars:
      1) residual projection along expected anomaly residual,
      2) residual norm,
      3) off-direction residual norm,
      4) anchor similarity margin.
    """
    ...
    def features(
        ...
        expected_delta = anomaly_anchor - action_anchor
        expected_delta_unit = F.normalize(expected_delta, dim=-1)
        along = (residual * expected_delta_unit).sum(dim=-1, keepdim=True)
        residual_norm = residual.norm(dim=-1, keepdim=True)
        off_direction = (residual - along * expected_delta_unit).norm(dim=-1, keepdim=True)
        sim_normal = (z * action_anchor).sum(dim=-1, keepdim=True)
        sim_anomaly = (z * anomaly_anchor).sum(dim=-1, keepdim=True)
        anchor_margin = sim_anomaly - sim_normal
        return torch.cat([along, residual_norm, off_direction, anchor_margin], dim=-1)
    def forward(
        self,
        z: torch.Tensor,
        actions: Sequence[str],
        ...
        return self.net(self.features(z, actions, action_cache, direction_head))
def add_residual_mlp_metrics(metrics: Dict[str, Any], output: "DirectionEvalOutput") -> Dict[str, Any]:
    metrics = dict(metrics)
    mlp_metrics = compute_binary_metrics(output.residual_mlp_y_true, output.residual_mlp_score, None, "balanced_accuracy")
    ensemble_metrics = compute_binary_metrics(output.residual_mlp_y_true, output.anchor_mlp_ensemble_score, None, "balanced_accuracy")
    for key in ["auroc", "auprc", "fpr_at_95_tpr", "score_mean", "score_std"]:
        metrics[f"residual_mlp_{key}"] = mlp_metrics.get(key, float("nan"))
        metrics[f"anchor_mlp_ensemble_{key}"] = ensemble_metrics.get(key, float("nan"))
    return metrics
...
def train_direction_epoch(
    motion_model: MotionAdapterEncoderNoProjector,
    direction_head: SharedAnomalyDirectionHead,
    residual_mlp: ResidualGeometryMLP,
    action_cache: ActionAnchorCache,
    loader: DataLoader,
    optimizer: torch.optim.Optimizer,
    device: torch.device,
    ...
    residual_mlp.train()
    scaler = torch.cuda.amp.GradScaler(enabled=use_amp)
    totals = {"loss": 0.0, "binary": 0.0, "direction": 0.0, "contrastive": 0.0, "residual_mlp": 0.0, "action": 0.0, "n": 0.0}
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            residual_mlp_logits = residual_mlp(z, actions_all, action_cache, direction_head)
            residual_mlp_loss = F.cross_entropy(residual_mlp_logits, labels)
            delta = h_a - h_h
            ...
            loss = (
                ...
                + float(lambda_residual_mlp) * residual_mlp_loss
            )
        scaler.scale(loss).backward()
        if grad_clip > 0:
            scaler.unscale_(optimizer)
            params = motion_model.trainable_parameters() + list(direction_head.parameters()) + list(residual_mlp.parameters())
        ...
        totals["residual_mlp"] += float(residual_mlp_loss.detach().cpu()) * n
...
class DirectionEvalOutput:
    ...
    residual_mlp_y_true: np.ndarray
    residual_mlp_score: np.ndarray
    anchor_mlp_ensemble_score: np.ndarray
@torch.no_grad()
def evaluate_direction_model(
    ...
    residual_mlp_y_true_all: List[int] = []
    residual_mlp_score_all: List[float] = []
    anchor_mlp_ensemble_score_all: List[float] = []
    total_loss = 0.0
    total_classification_loss = 0.0
    ...
    for batch in loader:
        ...
        probability = torch.softmax(logits, dim=-1)[:, 1]
        margin = sim_a - sim_n
        residual_mlp_logits = residual_mlp(z, batch["action"], action_cache, direction_head)
        residual_mlp_probability = torch.softmax(residual_mlp_logits, dim=-1)[:, 1]
        anchor_mlp_ensemble_probability = 0.5 * probability + 0.5 * residual_mlp_probability
        ...
        residual_mlp_y_true_all.extend(labels.detach().cpu().numpy().astype(int).tolist())
        residual_mlp_score_all.extend(residual_mlp_probability.detach().cpu().numpy().astype(float).tolist())
        anchor_mlp_ensemble_score_all.extend(anchor_mlp_ensemble_probability.detach().cpu().numpy().astype(float).tolist())
    return DirectionEvalOutput(
        ...
        residual_mlp_y_true=np.asarray(residual_mlp_y_true_all, dtype=int),
        residual_mlp_score=np.asarray(residual_mlp_score_all, dtype=float),
        anchor_mlp_ensemble_score=np.asarray(anchor_mlp_ensemble_score_all, dtype=float),
    )
...
def main() -> None:
    ...
    parser.add_argument("--lambda_residual_mlp", type=float, default=0.2,
                        help="Weight for the end-to-end tiny residual-geometry MLP auxiliary loss.")
    parser.add_argument("--residual_mlp_hidden_dim", type=int, default=16)
    parser.add_argument("--residual_mlp_dropout", type=float, default=0.1)
    parser.add_argument(
    ...
    residual_mlp = ResidualGeometryMLP(
        hidden_dim=args.residual_mlp_hidden_dim,
        dropout=args.residual_mlp_dropout,
    ).to(device)
    trainable_params = motion_model.trainable_parameters() + list(direction_head.parameters()) + list(residual_mlp.parameters())
    optimizer = torch.optim.AdamW(trainable_params, lr=args.lr, weight_decay=args.weight_decay)
    model_info = {
        "method": "shared_direction_with_end_to_end_tiny_residual_geometry_mlp",
        ...
        "residual_mlp_trainable": int(sum(p.numel() for p in residual_mlp.parameters())),
        ...
        "residual_geometry_mlp_auxiliary_loss_enabled": bool(args.lambda_residual_mlp > 0),
        "residual_geometry_mlp_features": ["along_expected_delta", "residual_norm", "off_direction_norm", "anchor_margin"],
    }
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        losses = train_direction_epoch(
            ...
            lambda_residual_mlp=args.lambda_residual_mlp,
            lambda_act=args.lambda_action,
            grad_clip=args.grad_clip,
            use_amp=args.amp,
        )
        train_output = evaluate_direction_model(motion_model, direction_head, residual_mlp, train_eval_loader, action_cache, device, args.temperature)
        val_output = evaluate_direction_model(motion_model, direction_head, residual_mlp, eval_loaders["val"], action_cache, device, args.temperature)
        train_metrics = add_residual_mlp_metrics(add_action_preservation_metrics(
            compute_binary_metrics(train_output.y_true, train_output.score, None, args.threshold_criterion), train_output
        ), train_output)
        val_metrics = add_residual_mlp_metrics(add_action_preservation_metrics(
        ...
        record = {
            ...
            "train_residual_mlp_loss": losses["residual_mlp"],
            ...
            "val_residual_mlp_auroc": val_metrics["residual_mlp_auroc"],
            "val_anchor_mlp_ensemble_auroc": val_metrics["anchor_mlp_ensemble_auroc"],
        ...
        print(
            ...
            f"mlp={losses['residual_mlp']:.4f} act_disabled={losses['action']:.4f} "
            f"val_loss={val_output.classification_loss:.6f} val_auroc={val_metrics['auroc']:.4f} "
            f"val_auprc={val_metrics['auprc']:.4f} val_mlp_auc={val_metrics['residual_mlp_auroc']:.4f} "
            f"val_ens_auc={val_metrics['anchor_mlp_ensemble_auroc']:.4f} val_action_acc={val_metrics['action_accuracy']:.4f} "
    ...
    if "residual_mlp" in ckpt:
        residual_mlp.load_state_dict(ckpt["residual_mlp"], strict=True)
    ...
    for name, loader in eval_loaders.items():
        output = evaluate_direction_model(motion_model, direction_head, residual_mlp, loader, action_cache, device, args.temperature)
        metrics = compute_binary_metrics(output.y_true, output.score, best_threshold, args.threshold_criterion)
        metrics = add_residual_mlp_metrics(add_action_preservation_metrics(metrics, output), output)
        ...
        prediction_df["residual_mlp_probability"] = output.residual_mlp_score
        prediction_df["anchor_mlp_ensemble_probability"] = output.anchor_mlp_ensemble_score
    ...
    final_summary = {
        ...
        "loss_weights": {
            ...
            "lambda_residual_mlp": float(args.lambda_residual_mlp),
