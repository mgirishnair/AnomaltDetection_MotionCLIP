...
uses paired healthy/anomaly batching, and saves direction/action-preservation diagnostics.
"""
...
def train_direction_epoch(
    ...
    scaler = torch.cuda.amp.GradScaler(enabled=use_amp)
    totals = {"loss": 0.0, "binary": 0.0, "direction": 0.0, "action": 0.0, "n": 0.0}
    for batch in loader:
        ...
        action_targets = _action_indices_from_action_cache(actions_all, action_cache, device)
        optimizer.zero_grad(set_to_none=True)
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss = float(lambda_bin) * binary_loss + float(lambda_dir) * direction_loss + float(lambda_act) * action_loss
        scaler.scale(loss).backward()
        if grad_clip > 0:
            scaler.unscale_(optimizer)
...
def evaluate_direction_model(
    ...
    for batch in loader:
        ...
        logits_action = action_preservation_logits(z, action_cache, temperature)
        action_loss = F.cross_entropy(logits_action, action_targets)
        loss = classification_loss + action_loss
        probability = torch.softmax(logits, dim=-1)[:, 1]
        margin = sim_a - sim_n
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and trains motion residual adapters plus one shared anomaly direction."
        )
    )
    # Data
    ...
    model_info = {
        "method": "shared_anomaly_direction_no_stage1_no_motion_projectors",
    ...
    print(f"[INFO] pair_table={len(pair_df)} pairs, pairs_per_batch={args.pairs_per_batch}, alpha_init={args.alpha_init}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        record = {
            ...
            "train_action_loss": losses["action"],
            "val_loss": val_output.classification_loss,
            "val_total_loss_binary_plus_action": val_output.loss,
            "val_action_loss": val_output.action_loss,
        ...
        print(
            ...
            f"dir={losses['direction']:.4f} act={losses['action']:.4f} "
            f"val_loss={val_output.classification_loss:.6f} val_auroc={val_metrics['auroc']:.4f} "
            f"val_auprc={val_metrics['auprc']:.4f} val_action_acc={val_metrics['action_accuracy']:.4f} "
    ...
    for name, loader in eval_loaders.items():
        ...
        metrics = add_action_preservation_metrics(metrics, output)
        metrics["loss"] = output.classification_loss
        metrics["total_loss_binary_plus_action"] = output.loss
        metrics["action_loss"] = output.action_loss
    ...
    final_summary = {
        ...
        "important_interpretation": {
            ...
            "action_preservation": "Uses all frozen action text anchors for multiclass action cross-entropy.",
