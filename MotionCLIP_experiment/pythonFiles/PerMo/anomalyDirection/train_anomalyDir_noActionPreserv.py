...
def train_direction_epoch(
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            loss = float(lambda_bin) * binary_loss + float(lambda_dir) * direction_loss
        scaler.scale(loss).backward()
        if grad_clip > 0:
            scaler.unscale_(optimizer)
...
def main() -> None:
    ...
    model_info = {
        "method": "shared_anomaly_direction_no_stage1_no_motion_projectors_no_action_preservation",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"dir={losses['direction']:.4f} act_disabled={losses['action']:.4f} "
