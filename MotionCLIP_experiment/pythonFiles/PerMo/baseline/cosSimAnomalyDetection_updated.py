...
def make_prompt_bank(
    ...
    """
    Creates exactly condition/action prompts.
    For each action and condition:
        <condition> <action>
    Example:
        healthy hop
        drunken hop
    ...
    conditions = sorted(set(normalize_text_label(c) for c in condition_prompt_labels))
    if normal_prompt_label not in conditions:
        raise ValueError(
            f"Normal prompt label '{normal_prompt_label}' not found in condition_prompt_label values: {conditions}"
        )
...
def score_samples(
    ...
    normal_prompt_label = normalize_text_label(normal_prompt_label)
    conditions = sorted(set(normalize_text_label(c) for c in condition_prompt_labels))
    nonhealthy_conditions = [c for c in conditions if c != normal_prompt_label]
...
def main():
    ...
    config["baseline_type"] = "Frozen MotionCLIP encoder + frozen CLIP text encoder + condition/action prompt cosine scoring"
    config["normal_definition"] = "is_anomaly == 0, expected to correspond to Healthy"
    config["anomaly_definition"] = "is_anomaly == 1, expected to correspond to non-Healthy conditions"
    config["main_score"] = "score_prompt_max_nonhealthy_minus_healthy"
    config["main_score_formula"] = "max_{condition != healthy} cos(motion, '<condition> <action>') - cos(motion, 'healthy <action>')"
    ...
    all_metrics["notes"] = {
        ...
        "score_prompt_max_nonhealthy_minus_healthy": (
            "max over non-healthy condition/action prompts cos(motion, '<condition> <action>') "
            "minus cos(motion, 'healthy <action>')"
        ),
