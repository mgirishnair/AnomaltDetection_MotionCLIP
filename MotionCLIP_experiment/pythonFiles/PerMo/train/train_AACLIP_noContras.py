...
MotionCLIP residual adapters and multi-level projectors using classification loss only. The script performs splitting,
...
@dataclass
class EvalOutput:
    # loss is the classification-only validation objective.
    loss: float
    # Kept as a separate field for compatibility with older analysis scripts.
    # It is identical to loss because this version does not use contrastive loss.
...
def train_stage2_epoch(
    ...
    """Train Stage 2 with classification loss only."""
    motion_model.train()
    scaler = torch.cuda.amp.GradScaler(enabled=use_amp)
    totals = {"loss": 0.0, "classification": 0.0, "n": 0.0}
    for batch in loader:
        motion = batch["motion"].to(device, non_blocking=True)
        labels = batch["label"].to(device, non_blocking=True)
        optimizer.zero_grad(set_to_none=True)
...
def main() -> None:
    ...
    parser.add_argument(
        "--stage2_contrastive_weight", type=float, default=0.0,
        help="Deprecated/ignored in this no-contrastive version; kept so old shell scripts still run.",
    )
    ...
    if float(args.stage2_contrastive_weight) != 0.0:
        print("[INFO] --stage2_contrastive_weight is ignored because this version trains without contrastive loss.")
    args.stage2_contrastive_weight = 0.0
    ...
    for epoch in range(1, args.stage2_epochs + 1):
        ...
        losses = train_stage2_epoch(
            ...
            args.temperature, args.grad_clip, args.amp,
        )
        ...
        print(
            ...
            f"val_loss={val_output.classification_loss:.6f} "
            f"val_auroc={val_metrics['auroc']:.4f} val_auprc={val_metrics['auprc']:.4f} "
    ...
    final_summary = {
        "method": "AA-MotionCLIP two-stage residual-adapter adaptation without contrastive loss",
        "stage1": {
