...
def build_prompt_cache(
    ...
    normal_class = norm_text(normal_class)
    if normal_class not in class_names:
        class_names = [normal_class] + class_names
    # Keep normal at index 0 for readability, but do not rely on this elsewhere.
    class_names = [normal_class] + sorted(c for c in set(class_names) if c != normal_class)
    ...
    for action in actions:
        ...
        for cls in class_names:
            if cls == normal_class:
                prompt = normal_prompt_template.format(action=action, normal_class=normal_class, class_name=cls)
            else:
                prompt = anomaly_prompt_template.format(action=action, anomaly_class=cls, class_name=cls, normal_class=normal_class)
...
def make_transfer_splits(args: argparse.Namespace, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, pd.DataFrame], Dict[str, Any]]:
    train_anomaly_classes = sorted(set(parse_list(args.train_anomaly_classes) + read_list_file(args.train_anomaly_classes_file)))
    test_anomaly_classes = sorted(set(parse_list(args.test_anomaly_classes) + read_list_file(args.test_anomaly_classes_file)))
    unseen_actions = sorted(set(parse_list(args.unseen_actions) + read_list_file(args.unseen_actions_file)))
    unseen_actors = sorted(set(parse_list(args.unseen_actors) + read_list_file(args.unseen_actors_file)))
    if not train_anomaly_classes:
        raise ValueError("Give at least one --train_anomaly_classes value, e.g. Drunken Exhausted.")
    if not test_anomaly_classes:
        raise ValueError("Give at least one --test_anomaly_classes value, e.g. Head-aching Text-necked.")
    available_classes = set(df["_class_norm"].unique().tolist())
    requested_classes = {args.normal_class} | set(train_anomaly_classes) | set(test_anomaly_classes)
    missing = sorted(requested_classes - available_classes)
    if missing:
        raise ValueError(f"Requested style/condition classes not found in {args.class_col}: {missing}. Available: {sorted(available_classes)}")
    overlap = sorted(set(train_anomaly_classes) & set(test_anomaly_classes))
    if overlap:
        raise ValueError(f"These classes are in both train and test anomaly sets: {overlap}")
    if args.normal_class in train_anomaly_classes or args.normal_class in test_anomaly_classes:
    ...
    # Only keep normal + selected anomaly styles.
    df = df[df["_class_norm"].isin(requested_classes)].copy()
    df["label"] = (df["_class_norm"] != args.normal_class).astype(int)
    df["anomaly_style_role"] = np.where(
        df["_class_norm"].eq(args.normal_class),
        "normal_class",
        np.where(df["_class_norm"].isin(train_anomaly_classes), "train_anomaly_class", "test_only_anomaly_class"),
    )
    ...
    # Seen training pool: normal + train anomaly styles only, excluding optional unseen actions/actors.
    seen_train_pool = df[
        (~optional_holdout_mask)
        & (df["_class_norm"].isin([args.normal_class] + train_anomaly_classes))
    ].copy()
    if not {0, 1}.issubset(set(seen_train_pool["label"].astype(int).unique())):
        raise ValueError("Training pool must contain both normal_class and train_anomaly_classes after optional holdouts.")
    ...
    # Test-only anomaly style split: all chosen test anomaly styles, excluding optional action/actor holdout.
    # This split is anomaly-only and is kept as a diagnostic/export split. Since it contains no
    # normal samples, AUROC/AUPRC will be NaN for this split by itself.
    unseen_anomaly_style_test = df[
        (~optional_holdout_mask)
        & (df["_class_norm"].isin(test_anomaly_classes))
    ].copy().reset_index(drop=True)
    # Main unseen-style evaluation split:
    #   label 0 = held-out normal_class samples from the same 80/20 stratified split used above
    #   label 1 = samples from the completely unseen anomaly styles
    # Because split_random_stratified_80_20() stratifies by _action_norm + _class_norm + label,
    # the held-out normal samples are approximately args.test_fraction per action for Healthy
    # whenever each action group has enough samples.
    heldout_normal_for_unseen_style = seen_test_df[
        seen_test_df["_class_norm"].eq(args.normal_class)
    ].copy().reset_index(drop=True)
    unseen_style_test = pd.concat(
        [heldout_normal_for_unseen_style, unseen_anomaly_style_test],
        axis=0,
    ).drop_duplicates(subset=["original_index"]).sample(
        frac=1.0, random_state=args.seed + 49
    ).reset_index(drop=True)
    # Optional action/actor test splits are separate diagnostic splits.
    test_sets: Dict[str, pd.DataFrame] = {
        "seen_style_test": seen_test_df.reset_index(drop=True),
        "unseen_style_test": unseen_style_test,
        "unseen_anomaly_style_test": unseen_anomaly_style_test,
    }
    if unseen_actions:
    ...
    test_sets["combined_test"] = combined
    # Leakage checks.
    train_classes = set(train_df["_class_norm"].unique())
    val_classes = set(val_df["_class_norm"].unique())
    leakage = {
        "test_anomaly_classes_in_train_val": sorted(set(test_anomaly_classes) & (train_classes | val_classes)),
    ...
    if args.balance_test_sets:
        ...
        test_sets = balanced
    summary = {
        "split_type": "unseen_anomaly_style_transfer",
        "class_col": args.class_col,
        "normal_class": args.normal_class,
        "train_anomaly_classes": train_anomaly_classes,
        "test_anomaly_classes": test_anomaly_classes,
        ...
        "unseen_style_test_definition": {
            "normal_samples": f"held-out {args.normal_class} samples from the seen-style test split",
            "anomaly_samples": test_anomaly_classes,
            "normal_holdout_fraction": float(args.test_fraction),
            "normal_holdout_stratification": stratify_cols,
        },
        "heldout_unseen_actions": unseen_actions,
        "heldout_unseen_actors": unseen_actors,
        "leakage_check": leakage,
        "test_balance_info": balance_info,
        "classes_used_for_prompts": [args.normal_class] + train_anomaly_classes + test_anomaly_classes,
...
def evaluate(
    ...
    row_indices: List[int] = []
    normal_idx = class_to_idx[norm_text(normal_class)]
    anomaly_prompt_classes = [norm_text(c) for c in anomaly_prompt_classes]
    anomaly_indices = [class_to_idx[c] for c in anomaly_prompt_classes if c in class_to_idx and c != norm_text(normal_class)]
    ...
    for batch in loader:
        ...
        if compute_contrastive_loss:
            # Only compute loss if every actual class in batch has a prompt. It should, because all filtered classes are in prompt cache.
            target_text, group_ids = target_text_embeddings_and_group_ids(batch["action"], batch["class_name"], action_to_idx, class_to_idx, text_feats, motion.device)
            loss = symmetric_motion_text_contrastive_loss(z, target_text, group_ids, temperature)
            losses.append(float(loss.detach().cpu()) * labels.numel())
        normal_logits = logits[:, normal_idx]
        anom_logits_all = logits.index_select(1, anomaly_indices_t)
        anom_max_logits, anom_argmax = anom_logits_all.max(dim=1)
        chosen_indices = anomaly_indices_t[anom_argmax].detach().cpu().numpy().astype(int).tolist()
        chosen_classes = [idx_to_class[i] for i in chosen_indices]
        score = anom_max_logits - normal_logits
        two_class_logits = torch.stack([normal_logits, anom_max_logits], dim=1)
        ...
        logit_normal.extend(normal_logits.cpu().numpy().astype(float).tolist())
        ...
        max_anom_classes.extend(chosen_classes)
...
def main() -> None:
    ...
    parser.add_argument("--normal_class", required=True, help="Style/condition treated as normal, e.g. Healthy.")
    ...
    if args.anomaly_class:
        args.train_anomaly_classes = list(args.train_anomaly_classes) + [args.anomaly_class]
    set_seed(args.seed)
    ...
    action_to_idx, class_to_idx, text_feats, prompt_info = build_prompt_cache(
        split_summary["actions_used_for_prompts"], prompt_classes, text_encoder, args.normal_class,
        args.normal_prompt_template, args.anomaly_prompt_template, device,
    )
    save_json(prompt_info, output_dir / "prompts.json")
    torch.save({"action_to_idx": action_to_idx, "class_to_idx": class_to_idx, "text_feats": text_feats.detach().cpu(), "prompt_info": prompt_info, "normal_class": args.normal_class, "anomaly_prompt_classes": [c for c in prompt_classes if c != args.normal_class]}, output_dir / "text_prompt_cache.pt")
    anomaly_prompt_classes = [c for c in prompt_classes if c != args.normal_class]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_eval_out = evaluate(motion_encoder, train_eval_loader, device, action_to_idx, class_to_idx, text_feats, args.normal_class, anomaly_prompt_classes, args.temperature, compute_contrastive_loss=False)
        ...
        val_out = evaluate(motion_encoder, val_loader, device, action_to_idx, class_to_idx, text_feats, args.normal_class, anomaly_prompt_classes, args.temperature, compute_contrastive_loss=True)
    ...
    train_out = evaluate(motion_encoder, train_eval_loader, device, action_to_idx, class_to_idx, text_feats, args.normal_class, anomaly_prompt_classes, args.temperature, True)
    val_out = evaluate(motion_encoder, val_loader, device, action_to_idx, class_to_idx, text_feats, args.normal_class, anomaly_prompt_classes, args.temperature, True)
    eval_outputs = {name: evaluate(motion_encoder, loader, device, action_to_idx, class_to_idx, text_feats, args.normal_class, anomaly_prompt_classes, args.temperature, True) for name, loader in test_loaders.items()}
    ...
    print("[DONE] Unseen style test metrics:")
    print(json.dumps(test_metrics_by_name.get("unseen_style_test", {}), indent=2, sort_keys=True))
    print("[DONE] Unseen anomaly-style diagnostic metrics:")
