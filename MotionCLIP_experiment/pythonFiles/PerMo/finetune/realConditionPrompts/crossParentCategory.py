...
def make_transfer_splits(args: argparse.Namespace, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, pd.DataFrame], Dict[str, Any]]:
    ...
    test_normal_classes = sorted(set(parse_list(args.test_normal_classes) + read_list_file(args.test_normal_classes_file)))
    if not test_normal_classes:
        test_normal_classes = [args.normal_class]
    all_normal_classes = sorted(set([args.normal_class] + test_normal_classes))
    unseen_actions = sorted(set(parse_list(args.unseen_actions) + read_list_file(args.unseen_actions_file)))
    unseen_actors = sorted(set(parse_list(args.unseen_actors) + read_list_file(args.unseen_actors_file)))
    ...
    requested_classes = set(all_normal_classes) | set(train_anomaly_classes) | set(test_anomaly_classes)
    ...
    normal_anomaly_overlap = sorted(set(all_normal_classes) & (set(train_anomaly_classes) | set(test_anomaly_classes)))
    if normal_anomaly_overlap:
        raise ValueError(f"Normal classes cannot also be anomaly classes: {normal_anomaly_overlap}")
    if unseen_actions:
    ...
    df["label"] = (~df["_class_norm"].isin(all_normal_classes)).astype(int)
    df["anomaly_style_role"] = np.where(
        df["_class_norm"].isin(all_normal_classes),
        "normal_class",
    ...
    # Cross-parent / test-only style split: test normal classes + test anomaly classes, excluding optional action/actor holdout.
    unseen_anomaly_style_test = df[
        (~optional_holdout_mask)
        & (df["_class_norm"].isin(test_normal_classes + test_anomaly_classes))
    ...
    leakage = {
        "test_normal_classes_in_train_val": sorted((set(test_normal_classes) - {args.normal_class}) & (train_classes | val_classes)),
    ...
    summary = {
        ...
        "class_col": args.class_col,
        "train_normal_class": args.normal_class,
        "test_normal_classes": test_normal_classes,
        ...
        "leakage_check": leakage,
        "test_balance_info": balance_info,
        "classes_used_for_prompts": all_normal_classes + train_anomaly_classes + test_anomaly_classes,
        "normal_prompt_classes": all_normal_classes,
...
def main() -> None:
    ...
    args.test_normal_classes = [norm_text(x) for x in args.test_normal_classes]
