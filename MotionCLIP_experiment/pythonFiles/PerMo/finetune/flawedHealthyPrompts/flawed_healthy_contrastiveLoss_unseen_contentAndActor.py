...
def select_unseen_actions(
    ...
    unseen_action_fraction: float = 0.0,
    require_both_labels: bool = True,
) -> List[str]:
...
def true_unseen_action_actor_split(
    df: pd.DataFrame,
    action_norm_col: str,
    actor_norm_col: str,
    condition_col: str,
...
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str], List[str], Dict[str, Any]]:
    """Split with truly held-out action/content and actor IDs, while using all styles/conditions.
    Train/val are created only from samples with seen action + seen actor.
    Styles/conditions are never held out: every style/condition remains eligible for train/val
    as long as its action and actor are not held out.
    Test buckets:
      - seen_test_df: seen action + seen actor
      - unseen_action_test_df: unseen action + seen actor
      - unseen_actor_test_df: seen action + unseen actor
      - unseen_action_actor_test_df: unseen action + unseen actor
    ...
    seen_mask = (~action_unseen) & (~actor_unseen)
    unseen_action_only_mask = action_unseen & (~actor_unseen)
    unseen_actor_only_mask = (~action_unseen) & actor_unseen
    unseen_action_actor_mask = action_unseen & actor_unseen
    ...
    unseen_action_actor_test_df = df[unseen_action_actor_mask].copy()
    if len(seen_df) == 0:
        raise ValueError("No seen data remains for training after action/actor holdout.")
    if not set(seen_df[label_col].astype(int).unique()).issuperset({0, 1}):
        raise ValueError("Seen training pool must contain both labels 0 and 1 after action/actor holdout.")
    if heldout_actions and len(unseen_action_test_df) == 0:
        print("[WARN] No isolated unseen-action rows remain. They may only exist in the unseen action+actor bucket.")
    if heldout_actors and len(unseen_actor_test_df) == 0:
        print("[WARN] No isolated unseen-actor rows remain. They may only exist in the unseen action+actor bucket.")
    if heldout_actions and heldout_actors and len(unseen_action_actor_test_df) == 0:
        print("[WARN] No rows exist where action and actor are both unseen at the same time.")
    # Stratify seen data by action + condition/style + actor + label so the regular seen test split
    # remains balanced across the available styles. This does not hold out styles.
    ...
    for part in [unseen_action_test_df, unseen_actor_test_df, unseen_action_actor_test_df]:
        if len(part) > 0:
    ...
    return (
        ...
        unseen_action_actor_test_df.sample(frac=1.0, random_state=seed + 9).reset_index(drop=True),
        combined_test_df,
        heldout_actions,
        heldout_actors,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune MotionCLIP with healthy/flawed prompts; hold out actions and/or actors, train on all styles.")
    # Data
    ...
    parser.add_argument("--condition_col", default="condition_label", help="Style/condition column. Used for stratifying seen splits only; styles are not held out.")
    ...
    # Split: only action/content and actor can be held out. All styles/conditions are used for train/val/test.
    parser.add_argument("--test_fraction", type=float, default=0.20, help="Seen-pool test fraction. Held-out actions/actors are entirely test-only.")
    ...
    parser.add_argument("--balance_test_sets", action="store_true", default=True, help="Downsample majority label in test sets so normal/anomaly counts are equal when possible.")
    parser.add_argument("--no_balance_test_sets", action="store_false", dest="balance_test_sets", help="Keep natural label ratios in test sets.")
    ...
    parser.add_argument("--unseen_action_fraction", type=float, default=0.0, help="Randomly hold out this fraction of action classes. Default 0 means no random action holdout.")
    ...
    # Styles are not held out. This column is only used for stratification/reporting when available.
    ...
    (
        ...
        unseen_action_actor_test_df,
        test_df,
        heldout_unseen_actions,
        heldout_unseen_actors,
        validation_fallback_info,
    ) = true_unseen_action_actor_split(
        df=df,
    ...
    # Hard leakage checks: no held-out action/actor may appear in train/val.
    ...
    test_balance_info = {
        ...
        "note": "If enabled, test splits with both labels are downsampled with a fixed seed so normal/anomaly counts are equal.",
    }
    if args.balance_test_sets:
        seen_test_df, seen_balance_info = safe_balance_binary_split(
            seen_test_df, args.label_col, args.seed + 100, "seen_action_seen_actor_test", allow_single_label=True
        )
        ...
        unseen_action_actor_test_df, unseen_action_actor_balance_info = safe_balance_binary_split(
            unseen_action_actor_test_df, args.label_col, args.seed + 350, "unseen_action_actor_test", allow_single_label=True
        )
        test_parts = [seen_test_df]
        for part in [unseen_action_test_df, unseen_actor_test_df, unseen_action_actor_test_df]:
            if len(part) > 0:
        ...
        test_balance_info.update({
            "seen_action_seen_actor_test": seen_balance_info,
            "unseen_action_only_test": unseen_action_balance_info,
            "unseen_actor_only_test": unseen_actor_balance_info,
            "unseen_action_actor_test": unseen_action_actor_balance_info,
    ...
    seen_test_df.to_csv(output_dir / "split_test_seen_action_seen_actor.csv", index=False)
    ...
    unseen_action_actor_test_df.to_csv(output_dir / "split_test_unseen_action_actor.csv", index=False)
    test_df.to_csv(output_dir / "split_test_combined.csv", index=False)
    test_df.to_csv(output_dir / "split_test.csv", index=False)
    split_summary = {
        "split_type": "true_unseen_action_actor_all_styles_seen",
        "style_policy": "all styles/conditions are available to train/val; styles are not held out",
        "total": int(len(df)),
        "train": int(len(train_df)),
        "val": int(len(val_df)),
        "seen_action_seen_actor_test": int(len(seen_test_df)),
        "unseen_action_only_test": int(len(unseen_action_test_df)),
        "unseen_actor_only_test": int(len(unseen_actor_test_df)),
        "unseen_action_actor_test": int(len(unseen_action_actor_test_df)),
        ...
        "unseen_action_actor_test_label_counts": unseen_action_actor_test_df[args.label_col].value_counts().to_dict(),
    ...
    unseen_action_actor_test_ds = PerMoMotionDataset(unseen_action_actor_test_df, args.path_col, args.action_col, args.label_col, args.motion_key, expected_shape)
    ...
    def make_loader(ds: Dataset, shuffle: bool = False) -> DataLoader:
        return DataLoader(ds, batch_size=args.batch_size, shuffle=shuffle, num_workers=args.num_workers, pin_memory=torch.cuda.is_available(), collate_fn=collate_batch)
    ...
    unseen_action_actor_test_loader = make_loader(unseen_action_actor_test_ds)
    # Model
    ...
    unseen_action_actor_test_out = evaluate(motion_encoder, unseen_action_actor_test_loader, device, action_to_idx, text_feats, args.temperature, True)
    ...
    unseen_action_actor_test_metrics = compute_binary_metrics(unseen_action_actor_test_out.y_true, unseen_action_actor_test_out.score, threshold=best_threshold)
    for m, out in [
        ...
        (unseen_action_actor_test_metrics, unseen_action_actor_test_out),
    ]:
        m["loss"] = out.loss
    # Save predictions
    ...
    save_predictions(seen_test_out, output_dir / "test_predictions_seen_action_seen_actor.csv", threshold=best_threshold)
    ...
    save_predictions(unseen_action_actor_test_out, output_dir / "test_predictions_unseen_action_actor.csv", threshold=best_threshold)
    ...
    save_embeddings(seen_test_out, output_dir / "test_embeddings_seen_action_seen_actor.npz", output_dir / "test_embeddings_seen_action_seen_actor_metadata.csv", threshold=best_threshold)
    ...
    save_embeddings(unseen_action_actor_test_out, output_dir / "test_embeddings_unseen_action_actor.npz", output_dir / "test_embeddings_unseen_action_actor_metadata.csv", threshold=best_threshold)
    ...
    final_summary = {
        ...
        "seen_action_seen_actor_test_metrics": seen_test_metrics,
        "unseen_action_test_metrics": unseen_action_test_metrics,
        "unseen_actor_test_metrics": unseen_actor_test_metrics,
        "unseen_action_actor_test_metrics": unseen_action_actor_test_metrics,
        ...
        "output_files": {
            ...
            "test_predictions_seen_action_seen_actor": str(output_dir / "test_predictions_seen_action_seen_actor.csv"),
            ...
            "test_predictions_unseen_action_actor": str(output_dir / "test_predictions_unseen_action_actor.csv"),
            ...
            "test_embeddings_seen_action_seen_actor": str(output_dir / "test_embeddings_seen_action_seen_actor.npz"),
            ...
            "test_embeddings_unseen_action_actor": str(output_dir / "test_embeddings_unseen_action_actor.npz"),
    ...
    print("[DONE] Seen action/actor test metrics:")
    ...
    print("[DONE] Hard test where action and actor are both unseen:")
    print(json.dumps(unseen_action_actor_test_metrics, indent=2, sort_keys=True))
