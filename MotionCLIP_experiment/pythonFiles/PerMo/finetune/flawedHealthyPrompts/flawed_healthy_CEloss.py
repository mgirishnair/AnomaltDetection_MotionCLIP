...
What this script does:
  ...
  5. Trains with cross-entropy over cosine similarities:
       logits = [cos(motion, text("healthy action")),
                 cos(motion, text("flawed action"))] / temperature
  6. Runs inference on a held-out test set.
  7. Saves metrics, predictions, split CSVs, and the best fine-tuned checkpoint.
Example:
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            logits, _ = logits_from_motion_and_prompts(
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pd.DataFrame(epoch_records).to_csv(output_dir / "epoch_metrics.csv", index=False)
        print(
            f"[EPOCH {epoch:03d}] "
    ...
    # Final eval
    train_out = evaluate(
        motion_encoder, train_loader, device, action_to_idx, text_feats, args.temperature, criterion
    )
    val_out = evaluate(
        motion_encoder, val_loader, device, action_to_idx, text_feats, args.temperature, criterion
    )
    test_out = evaluate(
        motion_encoder, test_loader, device, action_to_idx, text_feats, args.temperature, criterion
    )
