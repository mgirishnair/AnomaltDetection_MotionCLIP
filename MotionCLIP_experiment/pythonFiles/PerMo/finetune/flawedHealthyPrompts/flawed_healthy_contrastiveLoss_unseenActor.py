...
def true_unseen_action_style_actor_split(
    ...
    # If the per-group stratified validation split becomes empty, create a small global
    # validation split from the training pool while preserving both labels when possible.
    if len(val_df) == 0 and val_fraction > 0 and len(train_df) >= 4:
        rng = np.random.default_rng(seed + 12345)
        val_indices = []
        for lab, g in train_df.groupby(label_col):
            idxs = g.index.to_numpy()
            rng.shuffle(idxs)
            n_take = max(1, int(round(len(idxs) * val_fraction)))
            n_take = min(n_take, max(0, len(idxs) - 1))
            val_indices.extend(idxs[:n_take].tolist())
        if val_indices:
            val_df = train_df.loc[val_indices].copy().reset_index(drop=True)
            train_df = train_df.drop(index=val_indices).reset_index(drop=True)
    if len(val_df) == 0:
        raise ValueError(
            "Validation set is empty after splitting. Increase --val_fraction or reduce holdouts."
        )
    if not set(val_df[label_col].astype(int).unique()).issuperset({0, 1}):
        raise ValueError(
            "Validation set must contain both labels 0 and 1 so a threshold can be selected. "
            "Increase --val_fraction or reduce holdouts."
        )
...
def safe_compute_binary_metrics(
    ...
    """Compute metrics without crashing on empty or single-label diagnostic splits."""
    y_true = np.asarray(eval_out.y_true).astype(int)
    scores = np.asarray(eval_out.score).astype(float)
    if len(y_true) == 0:
        return {
            "split_name": split_name,
            "n_samples": 0,
            ...
            "threshold_source": "validation_threshold",
            "ranking_metrics_reported": False,
            "ranking_metrics_reason": "empty split",
        }
    if len(np.unique(y_true)) < 2 and allow_single_label:
        base = classification_metrics_at_threshold(y_true, scores, threshold)
        base.update({
            "split_name": split_name,
            "n_samples": int(len(y_true)),
            "n_normal": int((y_true == 0).sum()),
            ...
            "threshold_source": "validation_threshold",
            "ranking_metrics_reported": False,
            "ranking_metrics_reason": "single-label split; AUROC/AUPRC are not meaningful",
        })
        return base
    out = compute_binary_metrics(y_true, scores, threshold=threshold, threshold_criterion=threshold_criterion)
    out["split_name"] = split_name
    out["ranking_metrics_reported"] = bool(len(np.unique(y_true)) == 2)
    return out
# -----------------------------
# Train/evaluate
# -----------------------------
...
def main() -> None:
    ...
    parser.add_argument("--unseen_action_fraction", type=float, default=0.0, help="Randomly hold out this fraction of action classes. Default 0 means use all content/actions; only explicitly requested actions are held out.")
    ...
    unseen_action_test_metrics = safe_compute_binary_metrics(unseen_action_test_out, best_threshold, "unseen_action_only_test", args.threshold_criterion)
    unseen_actor_test_metrics = safe_compute_binary_metrics(unseen_actor_test_out, best_threshold, "unseen_actor_only_test", args.threshold_criterion)
    unseen_all_test_metrics = safe_compute_binary_metrics(unseen_all_test_out, best_threshold, "unseen_action_style_actor_test", args.threshold_criterion)
    other_unseen_combinations_metrics = safe_compute_binary_metrics(other_unseen_combinations_out, best_threshold, "other_unseen_combinations_test", args.threshold_criterion)
