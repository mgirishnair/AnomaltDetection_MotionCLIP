...
What this script does:
  ...
  1b. Holds out entire action classes from training/validation when requested.
  ...
  6. Runs inference on seen-action and truly unseen-action test sets using healthy/flawed prompt similarity.
...
def main() -> None:
    ...
    parser.add_argument("--unseen_actions", nargs="*", default=[], help="Action names to hold out completely from training/validation.")
    parser.add_argument("--unseen_actions_file", default="", help="Optional text file with one unseen action name per line.")
    ...
    train_df, val_df, seen_test_df, unseen_test_df, test_df, heldout_unseen_actions = true_unseen_action_split(
        df=df,
    ...
    # Hard leakage check: no action in train/val may appear in unseen test.
    train_actions = set(train_df["_action_norm"].unique().tolist())
    val_actions = set(val_df["_action_norm"].unique().tolist())
    unseen_actions_set = set(unseen_test_df["_action_norm"].unique().tolist())
    leakage = sorted((train_actions | val_actions) & unseen_actions_set)
    if leakage:
        raise RuntimeError(f"Unseen action leakage detected in train/val: {leakage}")
    test_balance_info = {
    ...
    if args.balance_test_sets:
        ...
        test_df = pd.concat([seen_test_df, unseen_test_df], axis=0).sample(
            frac=1.0,
        ...
        test_balance_info.update({
            "seen_action_test": seen_balance_info,
    ...
    split_summary = {
        "split_type": "true_unseen_action",
        "total": int(len(df)),
        "train": int(len(train_df)),
        "val": int(len(val_df)),
        "seen_action_test": int(len(seen_test_df)),
