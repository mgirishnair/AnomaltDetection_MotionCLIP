...
def select_unseen_actions(
    ...
    return chosen
def true_unseen_action_style_split(
    df: pd.DataFrame,
    action_norm_col: str,
    style_norm_col: str,
    condition_col: str,
...
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str], List[str]]:
    """Split so selected action/content and style/condition classes are test-only.
    Mutually exclusive test buckets:
      seen_test_df: seen actions and seen styles, sampled from the seen pool
      unseen_action_test_df: all samples whose action/content is held out
      unseen_style_test_df: samples whose action/content is seen, but style/condition is held out
      combined_test_df: seen_test + unseen_action_test + unseen_style_test
    This guarantees no held-out action or held-out style appears in training or validation.
    """
    ...
    unseen_styles_selected = select_unseen_values(
        df=df,
    ...
    unseen_style_set = set(unseen_styles_selected)
    unseen_action_test_df = df[df[action_norm_col].isin(unseen_set)].copy()
    # Keep test buckets mutually exclusive. Style holdout rows whose action is also unseen are
    # already in unseen_action_test_df.
    unseen_style_test_df = df[
        (~df[action_norm_col].isin(unseen_set)) &
        (df[style_norm_col].isin(unseen_style_set))
    ].copy()
    seen_df = df[
        (~df[action_norm_col].isin(unseen_set)) &
        (~df[style_norm_col].isin(unseen_style_set))
    ...
    if len(seen_df) == 0:
        raise ValueError("All rows were selected as unseen action/style; no seen data remains for training.")
    if len(unseen_action_test_df) == 0:
        raise ValueError("No unseen action/content test rows were selected.")
    if unseen_style_set and len(unseen_style_test_df) == 0:
        print(
            "[WARN] No exclusive unseen-style test rows remain. This can happen if all rows for "
            "the held-out styles also belong to held-out actions/content."
        )
    if not set(seen_df[label_col].astype(int).unique()).issuperset({0, 1}):
        raise ValueError("Seen training pool must contain both labels 0 and 1 after action/style holdout.")
    ...
    test_parts = [seen_test_df, unseen_action_test_df]
    if len(unseen_style_test_df) > 0:
        test_parts.append(unseen_style_test_df)
    combined_test_df = pd.concat(test_parts, axis=0).sample(frac=1.0, random_state=seed + 3).reset_index(drop=True)
    unseen_action_test_df = unseen_action_test_df.sample(frac=1.0, random_state=seed + 4).reset_index(drop=True)
    unseen_style_test_df = unseen_style_test_df.sample(frac=1.0, random_state=seed + 6).reset_index(drop=True)
...
def main() -> None:
    ...
    # Split
    parser.add_argument("--test_fraction", type=float, default=0.20, help="Seen-action test fraction. Unseen actions are entirely test-only.")
    parser.add_argument("--val_fraction", type=float, default=0.10, help="Fraction of non-test seen-action data used for validation.")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--balance_test_sets",
        action="store_true",
        default=True,
        help="Downsample majority label in seen, unseen, and combined test sets so normal/anomaly counts are equal.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--unseen_styles", nargs="*", default=[], help="Style/condition names to hold out completely from training/validation.")
    ...
    # Normalize labels/actions/styles
    ...
    if args.condition_col not in df.columns:
        if style_holdout_requested:
            raise ValueError(
                f"Style holdout was requested, but condition/style column {args.condition_col!r} "
                f"is missing. Found: {list(df.columns)}"
            )
    ...
    ) = true_unseen_action_style_split(
        df=df,
    ...
    # Hard leakage checks: no held-out action/style may appear in train/val.
    train_actions = set(train_df["_action_norm"].unique().tolist())
    val_actions = set(val_df["_action_norm"].unique().tolist())
    unseen_actions_set = set(heldout_unseen_actions)
    action_leakage = sorted((train_actions | val_actions) & unseen_actions_set)
    if action_leakage:
    ...
    unseen_styles_set = set(heldout_unseen_styles)
    style_leakage = sorted((train_styles | val_styles) & unseen_styles_set)
    if style_leakage:
        raise RuntimeError(f"Unseen style leakage detected in train/val: {style_leakage}")
    test_balance_info = {
        "enabled": bool(args.balance_test_sets),
        "note": "If enabled, the majority label is downsampled with a fixed seed so normal/anomaly counts are equal.",
    }
    if args.balance_test_sets:
        seen_test_df, seen_balance_info = balance_binary_split(
            seen_test_df,
            label_col=args.label_col,
            seed=args.seed + 100,
            split_name="seen_action_seen_style_test",
        )
        unseen_test_df, unseen_balance_info = balance_binary_split(
            unseen_test_df,
            label_col=args.label_col,
            seed=args.seed + 200,
            split_name="unseen_action_test",
        )

        balance_updates = {
            "seen_action_seen_style_test": seen_balance_info,
            "unseen_action_test": unseen_balance_info,
        }
        unseen_style_balance_info = {
            "split_name": "unseen_style_test",
            "balanced": False,
            "reason": "Unseen styles exclude Healthy, so this split contains anomaly samples only.",
            "n_samples": int(len(unseen_style_test_df)),
            "n_normal": int((unseen_style_test_df[args.label_col].astype(int) == 0).sum()),
            "n_anomaly": int((unseen_style_test_df[args.label_col].astype(int) == 1).sum()),
        }
        test_parts = [seen_test_df, unseen_test_df]
        if len(unseen_style_test_df) > 0:
            test_parts.append(unseen_style_test_df)
            balance_updates["unseen_style_test"] = unseen_style_balance_info
        test_df = pd.concat(test_parts, axis=0).sample(
            frac=1.0,
            random_state=args.seed + 300,
        ).reset_index(drop=True)
        test_df, combined_balance_info = balance_binary_split(
            test_df,
            label_col=args.label_col,
            seed=args.seed + 400,
            split_name="combined_test",
        )
        balance_updates["combined_test"] = combined_balance_info
        test_balance_info.update(balance_updates)
    ...
    # Keep old filename for compatibility; now this is the combined test set.
    test_df.to_csv(output_dir / "split_test.csv", index=False)
    split_summary = {
        "split_type": "true_unseen_action_and_optional_style",
        "total": int(len(df)),
        "train": int(len(train_df)),
        "val": int(len(val_df)),
        "seen_action_seen_style_test": int(len(seen_test_df)),
        "unseen_action_test": int(len(unseen_test_df)),
        "unseen_style_test": int(len(unseen_style_test_df)),
    ...
    unseen_test_ds = PerMoMotionDataset(unseen_test_df, args.path_col, args.action_col, args.label_col, args.motion_key, expected_shape)
    ...
    unseen_style_test_loader = DataLoader(
        unseen_style_test_ds,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
        pin_memory=torch.cuda.is_available(),
    ...
    seen_test_out = evaluate(
        motion_encoder, seen_test_loader, device, action_to_idx, text_feats, args.temperature, True
    )
    unseen_test_out = evaluate(
        motion_encoder, unseen_test_loader, device, action_to_idx, text_feats, args.temperature, True
    )
    unseen_style_test_out = evaluate(
        motion_encoder, unseen_style_test_loader, device, action_to_idx, text_feats, args.temperature, True
    )
    ...
    unseen_test_metrics = compute_binary_metrics(unseen_test_out.y_true, unseen_test_out.score, threshold=best_threshold)
    unseen_style_test_metrics = compute_binary_metrics(unseen_style_test_out.y_true, unseen_style_test_out.score, threshold=best_threshold)
    ...
    unseen_test_metrics["loss"] = unseen_test_out.loss
    unseen_style_test_metrics["loss"] = unseen_style_test_out.loss
    ...
    # Keep old filenames for compatibility; now this is the combined test set.
    ...
    final_summary = {
        ...
        "seen_action_test_metrics": seen_test_metrics,
        "unseen_action_test_metrics": unseen_test_metrics,
        ...
        "output_files": {
            ...
            "test_predictions_seen_actions": str(output_dir / "test_predictions_seen_actions.csv"),
            ...
            "test_embeddings_seen_actions": str(output_dir / "test_embeddings_seen_actions.npz"),
    ...
    print("[DONE] Seen-action test metrics:")
    print(json.dumps(seen_test_metrics, indent=2, sort_keys=True))
    print("[DONE] Truly unseen-action test metrics:")
    print(json.dumps(unseen_test_metrics, indent=2, sort_keys=True))
    if len(unseen_style_test_df) > 0:
        print("[DONE] Truly unseen-style test metrics:")
